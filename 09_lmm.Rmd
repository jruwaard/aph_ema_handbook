...
Figure \@ref(fig:fig7a) shows EMA mood ratings of the first 6 participants in the simulated data set. Mean mood ratings of the participants (the red lines) vary around 5 (the grey dashed line), as specified. 

```{r fig7a, out.width = "100%", fig.asp = .4, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = "Simulated EMA data of Six Participants."}
library(ggplot2)
...
```{r cs7a}
# code snippet 7.2: Fitting a mixed model with lme.
library(nlme)
...
```{r cs7b}
# code snippet 7.3: Print fixed effects.
summary(fm)$tTable
```
...
```{r cs7c}
# code snippet 7.4: Fitted random effects.
VarCorr(fm)
```
...
```{r cs7d7}
# code snippet 7.5: Saving predicted values.
d1$predY <- predict(fm)
```

```{r fig7b, out.width = "100%", fig.asp = 0.3, fig.cap = "EMA ratings, of each participant in the simulated data set, as predicted by the intercept-only mixed linear model.", echo = FALSE}
...
```{r}
# code snippet 7.6: Simulating ema data (time effect).
d2 <- sim_ema(plan, 
...
Figure \@ref(fig:fig7e) shows the data of the first six participants in the second data set. Both the intercept and the slope vary across the participants. Some participants improve more over time, and others improve less: the slope in this data set is a random effect.

```{r fig7e, out.width = "100%", fig.asp = .4, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = "Simulated EMA data of Six Participants (Time-varying model)."}
...
```{r cs7f}
# code snippet 7.7: A mixed model, with a random slope.
library(nlme)
...
```{r cs7g}
# code snippet 7.8: Extracting random effects.
VarCorr(fm)
```
...
```{r cs7h}
# code snippet 7.9: two-group simulation.
d3 <- rbind(d1, d2)
...
```{r cs7i}
# code snippet 7.10: A mixed model, with two groups.
library(nlme)
...
In Figure \@ref(fig:figk) below, EMA mood ratings predicted by the fitted model clearly show how the model detects 1) the fixed between-group effect, and 2) the variance in intercepts and slopes in both groups. 

```{r figk, echo = FALSE, out.width = "100%", fig.asp = .5, fig.cap = "Predicted mood ratings"}
d3$predY <- predict(fm)
...
The authoritative reference for mixed effect modeling in R is a book by Pinheiro and Bates [-@Pinheiro2000]. To fully appreciate this book, however, a strong background in formal statistics is required. Gentle introductions in the topic can be found in XXX, YYY and ZZZ ().   
