...
d$timestamp <- lubridate::round_date(d$timestamp, "30 seconds")
...
gps <- ggplot(subset(d, id == 1), aes(lon, lat)) +
   geom_point(alpha = .3,  shape = 21, size = 3) + 
...
```{r genea-one-hour, fig.asp = 0.5, fig.cap = "One hour of raw data collected with a wrist-worn GENEActiv accelerometer, sub-sampled to 10-second epochs (0.1 Hz)", echo = FALSE}
...
```{r genea-one-day, out.width = "100%", fig.asp = 0.35, fig.cap = "One day of data of the two persons in the GENEA data set of package 'emaph', summarised with ENMO, in 10-minute epochs", echo = FALSE}
...
ggplot(d, aes(time, svm)) +
  geom_hline(yintercept = 0.125, color = "red", linetype = 2) + 
  geom_line() + ylab("ENMO") + 
