...
Introduction, with Examples in R (1st edition - build 2018-11-26)}.
...
\index{EMA} \index{Ambulatory Assessment} \index{Experience Sampling}
\index{Ambulatory Self-reporting} \index{Real-time Data Capturing}
...
\index{Anxiety disorders} \index{Eating disorders}
\index{Mood disorders} \index{Substance-related disorders}
\index{Well-being}
...
\begin{Highlighting}[]
\CommentTok{# ------------------------------------------------------------}
\CommentTok{# Clean survey data }
\CommentTok{# JR - 2018-10-16}
\CommentTok{# ------------------------------------------------------------}

\CommentTok{# T0 (baseline) data -----------------------------------------}
\NormalTok{t0 <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"data/raw/surveys/sur_t0_2018_06_10.csv"}\NormalTok{)}

\CommentTok{# inject study ID, from study key file -----------------------}
\NormalTok{keys <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"data/raw/key_file.csv"}\NormalTok{)}

\KeywordTok{names}\NormalTok{(t0)[}\DecValTok{1}\NormalTok{] <-}\StringTok{ "Survey_ID"}
...
\CommentTok{# save cleaned T0 data --------------------------------- }
...
can be derived from the raw data, such as, e.g., survey sum-scores, or
actigraphy summary measures (see Chapter \ref{activity}). The example
below calculates the PHQ-9 sum scores from the item scores in the
cleaned baseline (t0) and post-test (t1) data:

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# ------------------------------------------------------------}
\CommentTok{# Pre-process survey data }
\CommentTok{# JR - 2018-10-16}
\CommentTok{# ------------------------------------------------------------}

\CommentTok{# import cleaned survey data ------------------------}
\KeywordTok{load}\NormalTok{(}\StringTok{"data/cleaned/t0.Rda"}\NormalTok{)}
\KeywordTok{load}\NormalTok{(}\StringTok{"data/cleaned/t1.Rda"}\NormalTok{)}

\CommentTok{# add PHQ-9 sum scores ---------------------------------------}
\NormalTok{t0}\OperatorTok{$}\NormalTok{phq9 <-}\StringTok{ }\KeywordTok{rowSums}\NormalTok{(t0[}\KeywordTok{paste0}\NormalTok{(}\StringTok{"phq"}\NormalTok{, }\DecValTok{1}\OperatorTok{:}\DecValTok{9}\NormalTok{)], }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{)}
\NormalTok{t1}\OperatorTok{$}\NormalTok{phq9 <-}\StringTok{ }\KeywordTok{rowSums}\NormalTok{(t1[}\KeywordTok{paste0}\NormalTok{(}\StringTok{"phq"}\NormalTok{, }\DecValTok{1}\OperatorTok{:}\DecValTok{9}\NormalTok{)], }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{)}

\CommentTok{# re-save baseline data --------------------------------------}
\KeywordTok{save}\NormalTok{(t0, }\DataTypeTok{file =} \StringTok{"data/cleaned/t0.Rda"}\NormalTok{)}
\KeywordTok{save}\NormalTok{(t1, }\DataTypeTok{file =} \StringTok{"data/cleaned/t1.Rda"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\index{Actigraphy} \index{Accelerometer} \index{GENEActiv}
...
\KeywordTok{library}\NormalTok{(ggplot2)}
\NormalTok{d <-}\StringTok{  }\KeywordTok{subset}\NormalTok{(emaph}\OperatorTok{::}\NormalTok{geneactiv, timestamp }\OperatorTok{>}\StringTok{ "2018-06-01 13:00"} \OperatorTok{&}
...
\CommentTok{# plot, with mean and variance ----------------------------}
\KeywordTok{ggplot}\NormalTok{(e, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ t, }\DataTypeTok{y =}\NormalTok{ s)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_hline}\NormalTok{(}\DataTypeTok{yintercept =} \KeywordTok{mean}\NormalTok{(e}\OperatorTok{$}\NormalTok{s, }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{), }
             \DataTypeTok{color =} \StringTok{"red"}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_hline}\NormalTok{(}
    \DataTypeTok{yintercept =} \KeywordTok{mean}\NormalTok{(e}\OperatorTok{$}\NormalTok{s, }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{) }\OperatorTok{+}\StringTok{ }\KeywordTok{sd}\NormalTok{(e}\OperatorTok{$}\NormalTok{s, }
...
\CommentTok{# The trends ----------------------------------------------}

\CommentTok{# run a linear regression of mood ratings by time}
...
\CommentTok{# imputation through interpolation}
...
\StringTok{  }\KeywordTok{spec.pgram}\NormalTok{(}
    \KeywordTok{ts}\NormalTok{(}\KeywordTok{impute_interpolation}\NormalTok{(d}\OperatorTok{$}\NormalTok{s), }
       \DataTypeTok{frequency =}\NormalTok{ n_measurements_per_day),}
...
\CommentTok{#> -2.68264 -0.58318  0.08341  0.72290  1.88910 }
\CommentTok{#> }
\CommentTok{#> Coefficients:}
\CommentTok{#>               Estimate Std. Error t value Pr(>|t|)   }
\CommentTok{#> activity.l1    0.30928    0.09881   3.130  0.00233 **}
\CommentTok{#> depression.l1 -0.04635    0.09697  -0.478  0.63376   }
\CommentTok{#> depression.l2 -0.17821    0.09672  -1.842  0.06856 . }
\CommentTok{#> const          0.11528    0.10578   1.090  0.27862   }
\CommentTok{#> ---}
\CommentTok{#> Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1}
\CommentTok{#> }
\CommentTok{#> Residual standard error: 1.027 on 94 degrees of freedom}
\CommentTok{#> Multiple R-squared:  0.1253, Adjusted R-squared:  0.09743 }
\CommentTok{#> F-statistic:  4.49 on 3 and 94 DF,  p-value: 0.005444}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{d <-}\StringTok{  }\KeywordTok{subset}\NormalTok{(geneactiv,}
...
\CommentTok{#> 1:     1  7 16.37369 36.66801}
\CommentTok{#> 2:     2  7 12.54895 26.95664}
\CommentTok{#> 3:     3  7 13.78576 32.03588}
\CommentTok{#> 4:     4  7 15.20732 38.84472}
\CommentTok{#> 5:     5  7 17.29907 40.60162}
\end{Highlighting}
\end{Shaded}
