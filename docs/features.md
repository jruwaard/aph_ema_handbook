...
@wiki\_Stationary\_process). If this assumption is incorrect (i.e., if the
...
@hoogendoorn2017\_book, for a discussion]. Figure \@ref(fig:fig8f) illustrates
...
are typically correlated with ratings taken at t-1, t-2, etc. This is
illustrated in Figure \@ref(fig:fig8g). Studying the auto-correlation of a
time-series at different delays can be very revealing, as illustrated by the
auto-correlation plot in Figure \@ref(fig:fig8g), in which the correlation of the
EMA series with itself is plotted, for various delays (or 'lags' as these delays
are called). First, the plot reveals that observations are correlated with
previous values at lag 1: this series is auto-correlated. Second, there appears
...
This series is "circadian": mood ratings seem to follow a daily and a weekly
pattern. This is a common characteristic of EMA data, because of the circadian
