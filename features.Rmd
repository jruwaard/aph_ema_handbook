...
EMA data are streams: time-series that represent processes which may span hours, days, weeks or even months. To the EMA researcher, it may not be immediately clear how to deal with these data. How, for example, can these series be summarized to study differences between individuals or changes within individuals over time? What 'qualities' or 'features' of a time-series should be considered? What are the available options? In this chapter, we will discuss some of the features that can be extracted from EMA time-series.
...
As can be seen in Figure \@ref(fig:fig8c), the regression model provides a more realistic and informative summary. At the start, the estimated mean - the intercept - is `r round(coef(fm)[1], 1)`. This mean increases approximately `r round(coef(fm)[2] * 7,  1)` points per day (the slope), to a final estimated mean of `r round(coef(fm)[1], 1) + round(coef(fm)[2] * 21, 1)`.
...
Now that we now that missing values are present, an new question arises: how should we deal with missing values? You can choose to ignore the missing values, or to replace the missing values with plausible values. Some analysis techniques, such as mixed modelling, do not require imputation, which is good. Others techniques, however, are intolerant to missing values. If you need to impute, there are several options. One can choose to replace missing values with the mean, the last-known value, an interpolated value or use a smoothing technique in which missing values are automatically replaced, such as the kalman-filter [see @hoogendoorn2017_book, for a discussion of these techniques]. Figure \@ref(fig:fig8f) illustrates the linear interpolation approach: missing values (marked by red dots) are replaced by values that lie between the non-missings.  
...
When a series is correlated with delayed copyies of itself, we say that it is autocorrelated. In repeated measurements of natural phenomena, this autocorrelation can often be found. The temperature of today is correlated with the temperature of recent days. Similarly, EMA mood ratings, taken at time t, are typically correlated with ratings taken at t-1, t-2, etc. This is illustrated in Figure \@ref(fig:fig8g). 
Studying the autocorrelation of a timeseries can be very revealing, as illustrated by the autocorrelation plot in Figure \@ref(fig:fig8g), in which the correlation of the EMA mood series with itself is plotted, for various delays (or 'lags' as these delays are called). First, the plot reveals that observations are correlated with previous values at lag 1: this series is autocorrelated. Second, there appears to be a pattern in the correlations at later lags: positive and negative correlations alternate, in a pattern that seems to reflect periodicity.   

```{r fig8g, echo = FALSE, out.width = "100%", fig.asp = .5, fig.cap = "Autocorrelation plot of the EMA mood series."}
acf(e$s, type = "partial", lag.max = 35)
```

### Rolling statistics

From the autocorrelation analysis, we learned that describing the EMA series as a simple trend might be too simplistic. The mean does not increase in a simple linear fashion. There seems to be a periodic component in the series as well: increases in the mean are followed by decreases. 

In the plot of the raw series, this is not immediately clear. However, if we "smooth" the series, by calculating the mean as it develops over time, the periodicity in the series is becoming more clear. Figure \@ref(fig:fig8h) illustrates this.

```{r fig8h, echo = FALSE, out.width = "100%", fig.asp = .3}
d$rolling_mean <-
  as.numeric(
    zoo::rollapply(
      as.numeric(d$s),
...
Both the autocorrelation analysis and the moving average plot suggested periodicity in the EMA time series. Through Fourier Analysis, the exact nature of the periodicity can be analysed. Figure \@ref(fig:fig8i) shows the strength ('power') of various frequencies in the EMA series.  

```{r fig8i, echo = FALSE, out.width = "100%", fig.asp = .6, fig.cap = "periodogram of the EMA time series, revealing a one-day and a one-week period."}
...
There are two peaks in the figure: at a frequency of 1 (day) and at a frequency of `r round(fa$freq[order(fa$spec, decreasing = TRUE)][1], 2)` (about one week). Mood ratings seem to follow a daily and a weekly pattern. The series is "circadian".
