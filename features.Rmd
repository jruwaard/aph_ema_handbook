...
```{r feat-plot, echo = FALSE, out.width = "100%", fig.asp = .4, fig.cap = "A simulated three-week time-series of EMA mood ratings."}
# plot signal
e <- subset(d, !is.na(s))
...
```{r feat-plot-mean, echo = FALSE, out.width = "100%", fig.asp = .3, fig.cap = "EMA time-series, with reference lines for the mean (red line) and the mean +/- 1 standard deviation range (the area between the two blue lines). Both statistics are informative, but obviously do not do full justice to the variation in observations over time."}
...
mean of `r round(coef(fm)[1], 1) + round(coef(fm)[2] * 21, 1)`. If you visually compare the red regression line in Figure \@ref(fig:feat-plot-lm) with the red line for the mean in \@ref(fig:feat-plot-mean), it is clear the regression line provides a much better summary.
...
fa <- spec.pgram(ts(impute_interpolation(d$s), frequency = 5), 
