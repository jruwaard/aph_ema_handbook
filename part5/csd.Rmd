...
We can now plot the mood state standard deviation and lag-1 autocorrelation. Did
...
fit_net <- function(d){
  nodes <- d %>% select(na_dt, pa_dt, mu_dt, wo_dt, su_dt, dayno)
  net <- graphicalVAR(data = nodes, gamma = 0, scale = FALSE,
                      nLambda = 40, verbose = FALSE,
                      dayvar = "dayno")
  
  plot(net, "PDC", title = FALSE, 
       labels = c("NA", "PA", "MU", "WO", "SU"),
       layout = "circle", theme = "classic", 
       vsize = 22, mar = c(10, 10, 10, 10),
       usePCH = TRUE, label.scale = FALSE)
}

layout(matrix(c(1:4), ncol=2, byrow = TRUE))
p1 <- fit_net(subset(csd, as.numeric(phase) == 1))
p2 <- fit_net(subset(csd, as.numeric(phase) == 2))
p3 <- fit_net(subset(csd, as.numeric(phase) == 3))
p4 <- fit_net(subset(csd, as.numeric(phase) == 4 & dayno < 127))
```
...
```{r}
library(qgraph)
d <- centralityTable(p1) %>% filter(grepl('Strength', measure)) %>% 
       select(node, value) %>% mutate(phase = 1)
d <- rbind(d,
       centralityTable(p2) %>% filter(grepl('Strength', measure)) %>% 
       select(node, value) %>% mutate(phase = 2))
d <- rbind(d,
       centralityTable(p3) %>% filter(grepl('Strength', measure)) %>% 
       select(node, value) %>% mutate(phase = 3))
d <- rbind(d,
       centralityTable(p4) %>% filter(grepl('Strength', measure)) %>% 
